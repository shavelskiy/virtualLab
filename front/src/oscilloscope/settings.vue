<template>
  <div class="card">
    ...
    <div class="card-body">
      ...
      <select class="form-control" @change="changeSettings" v-model.number="settings.voltDiv" :id="'voltsDiv-' + id">
      ...
      <input type="range" @input="changeSettings" v-model.number="settings.offsetX" :id="'offsetX-' + id" min="-500"
             max="500"/>

      <label :for="'offsetY-' + id">Сдвиг по вертикали</label>
      <input type="range" @input="changeSettings" v-model.number="settings.offsetY" :id="'offsetY-' + id" min="-300"
             max="300"/>
    </div>
  </div>
</template>
